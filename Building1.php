...
<body id="wrapper">
	...
	<div class="sidenav_heading">
		...
		<a href="Building1.php">Dashboard</a>
...
$sql = "SELECT * FROM ppw1112_youareondefault.Building";
...
<div class="heading">
				<h2 style="border-top: 2px solid #7D0552;">Complaints Records</h2>
				</div>
	...
	$sql = "SELECT distinct c.complaint_id,  a.apartment_id ,c.description FROM ppw1112_youareondefault.Complaint as c,ppw1112_youareondefault.Apartment as a, ppw1112_youareondefault.Building as b,Subdivision as s
	where c.apt_id=a.apartment_id and b.building_id=7";
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
	    // output data of each row
	    while($row = $result->fetch_assoc()) {
	...
	<div class="heading">
				<h2 style="border-top: 2px solid #7D0552;">Maintainance request Records</h2>
				</div>
	...
	$sql = "SELECT main_id, m.type, description FROM ppw1112_youareondefault.Maintenance as m, Apartment as a
where m.apt_id=a.apartment_id;";
	$result = $conn->query($sql);
	if ($result->num_rows > 0) {
	    // output data of each row
	    while($row = $result->fetch_assoc()) {
